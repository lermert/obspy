...
def cross_covar(data1, data2, max_lag_samples, normalize_traces=False):
    ...
    # Obtain the signal energy; the cross correlation is obtained from the cross covariance by normalizing with this value
    ...
    # A further diagnostic parameter to 'see' impulsive events: range of standard deviations
    nsmp = len(data1)/4
    std1 = np.zeros(4)
    std2 = np.zeros(4)
    for i in range(4):
