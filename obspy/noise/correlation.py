...
# coordinates, which are not in MSEED header --> metadata required; or default to some strange value?)
...
class Correlation(object):
    ...
    def __init__(self, stats_a=None, stats_b=None,
                 correlation=np.array([]),max_lag=0,correlation_type=None,
                 min_lag=None,n_stack=0,dist=0.,locked=False,
...
# Not sure if it makes more sense that 'add' creates a stack or a correlation stream.
# Should introduce creating a stream if IDs don't match (should now just raise error)
# The addition procedure is not yet well-solved I find. Hoping for better ideas here.
    def __add__(self, other):
        ...
        if self.n_stack == 0:
            ...
            if len(self.correlation) == len(other.correlation) and\
                ...
                #print("Old stack lengths %g and %g" %(self.n_stack,other.n_stack))
                self.n_stack+=other.n_stack
    ...
    def __str__(self):
        ...
        wins = '  |  %g windows'
        out = corrtype + self.id + \
            Fs % (self.stats_a) + lag % (self.max_lag) +\
             wins %(self.n_stack)
        return out
    ...
    def plot(self):
        ...
        if self.min_lag == 0 or self.min_lag == None:
    ...
    def filter(self, type, **options):
        ...
        self.correlation = func(self.correlation, df=self.stats_a.sampling_rate,\
        **options)
        return self
    ...
    def save(self,filename,format):
        ...
        if format =='SAC':
            ...
            tr.stats.sac['kt8']=self.correlation_type if self.correlation_type\
            else ''
...
class CorrelationStream(object):
    ...
    def __getitem__(self, index):
        ...
        if isinstance(index, slice):
            return self.__class__(correlations=self.__correlations.__getitem__(index))
        else:
...
# A proper solution is needed here. This is just temporary..
    ...
    def stack(self,station1=None,station2=None,location1=None,\
               location2=None,channel1=None,channel2=None,n=None,noloczeroloc=False):
        ...
        for corr in self.select(station1,station2,location1,\
                    location2,channel1,channel2,noloczeroloc).__correlations[0:n]:
            try:
                #stacks retaining the stats and updating the n_stack
                stack += corr
                
            except ValueError:
                print("At least one does not match: max. lag, delta, station1, station2")
                continue
        return stack
    ...
    def select(self,station1=None,station2=None,location1=None,\
        ...
        for corr in self.__correlations:
            ...
            if channel1 is not None:
                if not corr.stats_a.channel in channel1:
                    continue
                    
            if channel2 is not None:
                if not corr.stats_b.channel in channel2:
                    continue
    ...
    def sort(self, keys=['stadist','network', 'station', 'location', 'channel'],\
                        reverse=False):
            """
            ...
            # Normally, the distance between the stations is handled as a variable of the Correlation. If needed for sorting, add it  
            if 'stadist' in keys:
                for corr in self.__correlations:
    ...
    def plot(self,maxlag=None):
        ...
        if len(self.__correlations) == 1:
            ...
            self.sort()
            if maxlag == None: maxlag = self.__correlations[0].max_lag
            scaling = 0.01 * self.__correlations[-1].dist/1000.
            
            for corr in self.__correlations:
                if True in np.isnan(corr.correlation): continue
                if True in np.isinf(corr.correlation): continue
                lag = np.linspace(-corr.max_lag, corr.max_lag,\
                len(corr.correlation))
                plt.plot(lag, scaling * corr.correlation/\
                np.max(np.abs(corr.correlation))\
                + corr.dist/1000., 'k')
                plt.xlabel('Lag (s)')
                plt.tick_params(
                    axis='y',
...
def correlate_trace(trace_a, trace_b, max_lag, correlation_type, **kwargs):
    ...
    corr = Correlation(trace_a.stats.copy(), trace_b.stats.copy(),
                       max_lag=max_lag,
                       correlation=corr,
